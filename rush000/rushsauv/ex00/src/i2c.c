...
void i2c_write(uint8_t data, uint8_t ack, uint8_t nack)
{
    TWDR = data; // Charger la donnée dans le registre de données TWDR
    TWCR = (1 << TWINT) | (1 << TWEN) | (ack << TWEA) | (nack << TWSTO); // Déclencher la transmission
    while (!(TWCR & (1 << TWINT)))
        uart_printstr("Write\r\n"); // Attente de l'achèvement de l'opération
}
...
void i2c_read(uint8_t ack, uint8_t nack)
{
    TWCR = (1 << TWINT) | (1 << TWEN) | (ack << TWEA) | (nack << TWSTO); // Déclencher la lecture
    while (!(TWCR & (1 << TWINT)))
        uart_printstr("Read\r\n"); // Attente de l'achèvement de l'opération
}
